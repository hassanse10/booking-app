    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          ...
          <button onClick={() => navigate('/teacher')} className="mt-4 px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm">
            ← Retour
          </button>
        </div>
      </div>
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-white border-b border-gray-200 sticky top-0 z-30">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-4 flex items-center gap-4">
          ...
          <div>
            <h1 className="font-bold text-gray-900">Tableau de Bord Analytique</h1>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <StatCard icon="📅" label="Séances confirmées" value={totalSessions} color="indigo" />
          <StatCard icon="⏱" label="Durée moyenne" value={avgDuration ? `${avgDuration} min` : '—'} color="amber" />
          <StatCard icon="❌" label="Taux d'annulation" value={`${cancelRate}%`} color={cancelRate > 20 ? 'red' : 'indigo'} />
        </div>
                ...
                {(revenue.studentBreakdown ?? []).slice(0, 5).map((s) => (
                  <div key={s.id} className="flex items-center gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">{s.sessions} séance{s.sessions > 1 ? 's' : ''}</p>
                    </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="space-y-2">
                {engagement.mostActiveStudents.map((s, i) => (
                  <div key={s.id} className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-indigo-600">{s.bookings} séances</span>
                  </div>
                ))}
              </div>
            )}
          ...
          <Section title="💤 Élèves inactifs (+30 jours)">
            {(engagement?.inactiveStudents ?? []).length === 0 ? (
              <p className="text-gray-400 text-sm">Tous vos élèves sont actifs 🎉</p>
            ) : (
              <div className="space-y-2">
          ...
          <Section title="📊 Statistiques Réservations">
            <div className="space-y-4">
              ...
              <div className="pt-2 border-t border-gray-100">
                <p className="text-xs text-gray-500">Durée moy. de séance</p>
                <p className="text-xl font-bold text-indigo-600">{avgDuration} min</p>
              </div>
            </div>
          </Section>
        </div>
        ...
        <Section title="🕐 Heures de Pointe (créneaux réservés)">
          {maxPeak === 0 ? (
            <p className="text-gray-400 text-sm text-center py-6">Aucune séance enregistrée</p>
          ) : (
            <div className="overflow-x-auto">
              <div className="min-w-max">
                ...
                <div className="flex gap-1 mb-1 pl-10">
                  {HOURS.map(h => (
                    <div key={h} className="w-8 text-center text-xs text-gray-400">{h}h</div>
                  ))}
                </div>
                {DAYS.map((day, dayIdx) => (
                  <div key={dayIdx} className="flex items-center gap-1 mb-1">
                    <div className="w-8 text-xs font-medium text-gray-500 text-right pr-2">{day}</div>
                    {HOURS.map(h => {
                      const count = heatmap[dayIdx]?.[h] ?? 0;
                      const intensity = count === 0 ? 0 : Math.ceil((count / maxPeak) * 4);
                      const bgMap = ['bg-gray-100', 'bg-indigo-100', 'bg-indigo-200', 'bg-indigo-400', 'bg-indigo-600'];
                      return (
              ...
              {funnelData.map((stage, i) => (
                <div key={i}>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="font-bold text-gray-800">{stage.count} <span className="text-gray-400 font-normal">({stage.percentage}%)</span></span>
                  </div>
            ...
            <div className="space-y-2">
              {(invoices.invoices ?? []).slice(0, 5).map((inv) => (
                <div key={inv.id} className="flex justify-between items-center bg-white rounded-xl px-4 py-3 border border-red-100">
                  <div>
                    <p className="font-semibold text-gray-800 text-sm">{inv.first_name} {inv.last_name}</p>
                    <p className="text-xs text-gray-400">{inv.email}</p>
                  </div>
                  <div className="text-right">
                    <p className="font-bold text-red-600">{parseFloat(inv.amount).toFixed(2)}€</p>
                    <p className="text-xs text-gray-400">Échu le {new Date(inv.due_at).toLocaleDateString('fr-FR')}</p>
                  </div>
                </div>
              ))}
            </div>
